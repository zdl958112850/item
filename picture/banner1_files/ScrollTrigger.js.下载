	...
	return function(defaultOptions, bindTo, scrollIn) {
		...
		this.scrollDidChange = function(_this) {
			return function() {
				...
				triggers.each(function(trigger, index){
					// var triggerLeft = trigger.left();	暂时注释掉 zqy
					// var triggerTop = trigger.top();	暂时注释掉 zqy
					var triggerLeft = trigger.element.parentNode.getBoundingClientRect().left;
					var triggerTop = trigger.element.parentNode.getBoundingClientRect().top;
